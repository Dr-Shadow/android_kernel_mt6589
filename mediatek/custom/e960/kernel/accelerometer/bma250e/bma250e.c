...
static int BMA250_ReadData(struct i2c_client *client, s16 data[BMA250_AXES_NUM])
{
	...
	else
	{
		...
		if(0) //hongzhe-dbg if(1)//(atomic_read(&priv->trace) & ADX_TRC_RAWDATA)
		{
...
static int BMA250_CheckDeviceID(struct i2c_client *client)
{
	...
	if(databuf[0]!=BMA250_FIXED_DEVID && databuf[0]!=BMA250E_FIXED_DEVID)
	{
...
#endif 
printk("[SENSOR - HONGZHE]%s %d probe ok\n", __func__, __LINE__);
	GSE_LOG("%s: OK\n", __func__);    
	return 0;
