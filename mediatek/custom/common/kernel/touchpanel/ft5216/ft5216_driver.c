...
#define VELOCITY_CUSTOM_FT5216
#ifdef VELOCITY_CUSTOM_FT5216
...
struct touch_info {
 ...
 static const struct i2c_device_id ft5216_tpd_id[] = {{"ft5216",0},{}};
 //unsigned short force[] = {0,0x70,I2C_CLIENT_END,I2C_CLIENT_END}; 
 //static const unsigned short * const forces[] = { force, NULL };
 //static struct i2c_client_address_data addr_data = { .forces = forces, };
 static struct i2c_board_info __initdata ft5216_i2c_tpd={ I2C_BOARD_INFO("ft5216", (0x70>>1))};
 
 
 static struct i2c_driver tpd_i2c_driver = {
  .driver = {
	 .name = "ft5216",//.name = TPD_DEVICE,
//	 .owner = THIS_MODULE,
  },
  .probe = tpd_probe,
  .remove = __devexit_p(tpd_remove),
  .id_table = ft5216_tpd_id,
  .detect = tpd_detect,
	  ...
	  mt65xx_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM); 
		 ...
		 set_current_state(TASK_RUNNING);
		  ...
		  if (tpd_touchinfo(&cinfo, &pinfo)) 
		  {
			...
			if(point_num >0) 
			{
			    for(i =0; i<point_num && i<5; i++)//only support 3 point //5216-5 point
			    {

			         tpd_down(cinfo.x[i], cinfo.y[i], cinfo.id[i]);
			       TPD_DMESG("ft5216: %d %d %d",cinfo.x[i], cinfo.y[i], cinfo.id[i] );
			    }
			    input_sync(tpd->dev);
			}
...
mdelay(100);
	...
	TPD_DMESG("ft5216 Touch Panel Device Probe %s\n", (retval < TPD_OK) ? "FAIL" : "PASS");
   return 0;
 ...
 static int tpd_local_init(void)
 {
  ...
  TPD_DMESG("Focaltech FT5216 I2C Touchscreen Driver (Built %s @ %s)\n", __DATE__, __TIME__);
 
 
   if(i2c_add_driver(&tpd_i2c_driver)!=0)
   	{
  		TPD_DMESG("ft5216 unable to add i2c driver.\n");
      	return -1;
    }
    if(tpd_load_status == 0) 
    {
    	TPD_DMESG("ft5216 add error touch panel driver.\n");
...
i2c_smbus_write_i2c_block_data(i2c_client, 0xA5, 1, &data);  //TP enter sleep mode
 ...
 static int __init tpd_driver_init(void) {
	 printk("MediaTek FT5216 touch panel driver init\n");
	   i2c_register_board_info(0, &ft5216_i2c_tpd, 1);
		 if(tpd_driver_add(&tpd_device_driver) < 0)
			 TPD_DMESG("add FT5216 driver failed\n");
	 return 0;
 }
 
 /* should never be called */
 static void __exit tpd_driver_exit(void) {
	 TPD_DMESG("MediaTek FT5216 touch panel driver exit\n");
