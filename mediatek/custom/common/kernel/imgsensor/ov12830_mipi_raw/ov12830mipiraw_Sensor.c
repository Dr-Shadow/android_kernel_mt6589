...
#define OV12830_ORIENTATION   (IMAGE_NORMAL)

#ifdef OV12830_DEBUG
	//#define OV12830DB(fmt, arg...) xlog_printk(ANDROID_LOG_DEBUG, "[OV12830Raw] ",  fmt, ##arg)
	#define OV12830DB(fmt, arg...) printk("[OV12830Raw] ",  fmt, ##arg)
#else
...
#define OV12830MIPI_WRITE_ID_2   0x20

u16 OV12830MIPI_WRITE_ID = OV12830MIPI_WRITE_ID_1;
u16 OV12830MIPI_READ_ID = OV12830MIPI_WRITE_ID_1+1;
...
#define OV12830_write_cmos_sensor_v2(addr, para) iWriteReg((u16) addr , (u32) para , 1, OV12830MIPI_WRITE_ID_2)
kal_uint16 OV12830_read_cmos_sensor_v2(kal_uint32 addr)
{
kal_uint16 get_byte=0;
    iReadReg((u16) addr ,(u8*)&get_byte,OV12830MIPI_WRITE_ID_2);
    return get_byte;
}
...
0x01,0x03,0x01,
0x30,0x01,0x66, // <---- hongzhe 0x06->0x66
0x30,0x02,0x80,
    ...
    do {
        *sensorID = (OV12830_read_cmos_sensor(0x300A)<<8)|OV12830_read_cmos_sensor(0x300B);
        if (*sensorID == OV12830_SENSOR_ID)
        	{
        		printk("[SENSOR-HONGZHE]Sensor ID = 0x%04x\n", *sensorID);
            	break;
        	}
        printk("[SENSOR-HONGZHE]Read Sensor ID Fail = 0x%04x\n", *sensorID);
        retry--;
    } while (retry > 0);

#if 1 //hongzhe.liu for multi address check
    if (*sensorID != OV12830_SENSOR_ID) {

		printk("OV12830GetSensorID enter 2 :\n ");
		OV12830_write_cmos_sensor_v2(0x0103,0x01);// Reset sensor
	    mDELAY(10);

	    // check if sensor ID correct
	    do {
	        *sensorID = (OV12830_read_cmos_sensor_v2(0x300A)<<8)|OV12830_read_cmos_sensor_v2(0x300B);
	        if (*sensorID == OV12830_SENSOR_ID)
	        	{
	    ...
	    OV12830MIPI_WRITE_ID = OV12830MIPI_WRITE_ID_2;
	    OV12830MIPI_READ_ID = OV12830MIPI_WRITE_ID_2+1;
