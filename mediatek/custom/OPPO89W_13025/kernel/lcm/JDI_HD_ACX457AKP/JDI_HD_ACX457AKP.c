...
#if 0
static struct LCM_setting_table lcm_truly_new_initialization_setting[] = {
...
static struct LCM_setting_table lcm_truly_new_initialization_setting[] = {
	...
	{0xBC,	1,	{0x07}},    //SLEEP OUT
	{0xBA,	1,	{0x13}}, 
	{0xB1,	16,	{0x01,0x00,0x07,0x87,0x01,0x11,0x11,0x2A,0x30,0x3F,
                 0x3F,0x47,0x12,0x01,0xE6,0xE2}},//0x29, 0x29,
	{0xB2,	6,	{0x00,0xC8,0x08,0x04,0x00,0x22}},
	{0xD5,	32,	{0x00,0x00,0x00,0x00,0x0A,0x00,0x01,0x00,0xCC,0x00,
				0x00,0x00,0x88,0x88,0x88,0x88,0x88,0x88,0x88,0x88,
				0x88,0x88,0x01,0x67,0x45,0x23,0x01,0x23,0x88,0x88,
...
#endif
static struct LCM_setting_table lcm_truly_old_initialization_setting[] = {
...
static void lcm_get_id_pin()
{
		...
		int tmp0 = 0,tmp1 = 1,tmp2 = 0;
		mt_set_gpio_mode(133, 0); 
		mt_set_gpio_dir(133, 0);
		mt_set_gpio_pull_select(133,1);
		mt_set_gpio_pull_enable(133,1);
    
		mt_set_gpio_mode(GPIOEXT18, 0); 
		mt_set_gpio_dir(GPIOEXT18, 0);
		mt_set_gpio_pull_select(GPIOEXT18,1);
		mt_set_gpio_pull_enable(GPIOEXT18,1);
		
		mt_set_gpio_mode(GPIOEXT19, 0); 
		mt_set_gpio_dir(GPIOEXT19, 0);
		mt_set_gpio_pull_select(GPIOEXT19,1);
		mt_set_gpio_pull_enable(GPIOEXT19,1);
		
		
		tmp0 = mt_get_gpio_in(133);
		tmp1 = mt_get_gpio_in(GPIOEXT18);
		tmp2 = mt_get_gpio_in(GPIOEXT19);
		DMS_LCD("LCD get id: tmp0 = %d,tmp1 = %d,tmp2 = %d\n",tmp0,tmp1,tmp2);  
		if(tmp0==1&&tmp1==0&&tmp2==0)
		{
				g_lcd_id_value3 = TRULY_OLD;
		}
...
static void lcm_init(void)
{
   ...
   if(g_lcd_id_value3 == TRULY_OLD)
   {	
		push_table(lcm_truly_old_initialization_setting, sizeof(lcm_truly_old_initialization_setting) / sizeof(struct LCM_setting_table), 1);
	 
    } 
   else if(g_lcd_id_value3 == TRULY_NEW)
   {
		push_table(lcm_truly_new_initialization_setting, sizeof(lcm_truly_new_initialization_setting) / sizeof(struct LCM_setting_table), 1);
   }
   else if(g_lcd_id_value3 == JDI)
   {
...
static void lcm_suspend(void)
{
   //#ifdef UBOOT_LK
	...
	if(g_lcd_id_value3 == TRULY_OLD || g_lcd_id_value3 == TRULY_NEW)
	{
