...
static struct LCM_setting_table lcm_cmd_truly_initialization_setting[] = {	
	...
	*/
	...
	{0xB1,	19,	{0x01, 0x00, 0x34, 0x06,0x00,
				 0x0f, 0x0f, 0x2a, 0x32,
				 0x3f, 0x3f, 0x07,0x13, //0x3f,0x3f
				 0x01, 0xE6, 0xE6,
				 0xE6, 0xE6, 0xE6}},
		{0xB2,	15, {0x00, 0x20, 0x0a, 0x0a,//if cmd mode 0x20 if vdo mode 0x23
				 0xd0, 0x00, 0xFF, 0x00, //0x70
				 0x00, 0x00, 0x00, 0x03,
				 0x03, 0x00, 0x01}},
	{0xB4, 	5,	{0x00, 0x0c, 0x84, 0x0c, ////////0x0a, 0x0c, 0x84, 0x0c,
				0x01}},
	{0xB6,	2,	{0x2f, 0x2f}}, //0x27,0x27//flick
	...
	{0xE0,	34,	{0x00, 0x08, 0x0d, 0x2d,
				 ...
				 0x13,0x19,}},
	{REGFLAG_DELAY, 1, {}}, 
	{0xBA,	15,	{0x00, 0xA0, 0xC6, 0x80,
				 0x1A, 0x00, 0x10, 0x30,
				 0x6c,0x02,0x11,0x18,//0x6c,0x02,0x11,0x18
				 0x40,0x00,0x00}},
...
static struct LCM_setting_table lcm_cmd_byd_initialization_setting[] = {
	#if 1
	{0xb9,	3,	{0xFF, 0x83, 0x69}},
	{0xb1,	19,	{0x01,0x00,0x34,0x07,0x00,0x0E,0x0E,0x1A,0x22,0x3F,
	                     0x3F,0x07,0x0b,0x01,0xE6,0xE6,0xE6,0xE6,0xE6}},
    {REGFLAG_DELAY, 1, {}},
	{0xb2,	15,	{0x00,0x20,0x0A,0x0A,0x70,0x00,0xFF,0x00,0x00,0x00, //if cmd mode 0x20 if vdo mode 0x23
...
static struct LCM_setting_table lcm_vdo_truly_initialization_setting[] = {
#if 1
	{0xB9,	3,	{0xFF, 0x83, 0x69}},
	{0xB1,	19,	{0x01, 0x00, 0x34, 0x06,0x00,
				 0x0f, 0x0f, 0x2a, 0x32,
...
static struct LCM_setting_table lcm_cmd_sleep_mode_in_setting[] = {
...
static void lcm_get_id_pin()
{
    ...
    int tmp0 = 0,tmp1 = 0;  
		int ret;
		mt_set_gpio_mode(250, 0); 
		mt_set_gpio_dir(250, 0);
		mt_set_gpio_pull_select(250,0);
		mt_set_gpio_pull_enable(250,1);
    
		mt_set_gpio_mode(251, 0); 
		mt_set_gpio_dir(251, 0);
		mt_set_gpio_pull_select(251,0);
		mt_set_gpio_pull_enable(251,1);
		tmp0 = mt_get_gpio_in(250);
		tmp1 = mt_get_gpio_in(251);
		
		DMS_LCD("LCD get id: tmp0 = %d,tmp1 = %d\n",tmp0,tmp1);  
		if(tmp0&&tmp1)
			{
    ...
    {
			DMS_LCD("This is truly LCD g_lcd_id_value3 = %d\n",g_lcd_id_value3);
#ifdef CMD_MODE_USED
			ret = mt_set_gpio_mode(148, 0); 
			ret = mt_set_gpio_dir(148, 1);
			ret = mt_set_gpio_out(148,0);
			DMS_LCD("LCD get id: gpio148_lvl = %d\n",ret);
			DMS_LCD("LCD get id: cmd_mode_used\n");
#else
			ret = mt_set_gpio_mode(148, 0); 
			ret = mt_set_gpio_dir(148, 1);
			ret = mt_set_gpio_out(148,1);
			DMS_LCD("LCD get id: gpio148_lvl = %d\n",ret);
			DMS_LCD("LCD get id: vdo_mode_used\n");
#endif
    }
		else
    {
			DMS_LCD("This is BYD LCD g_lcd_id_value3 = %d\n",g_lcd_id_value3);
...
static void lcm_get_params(LCM_PARAMS *params)
{
       ...
       // Istruly = TRULY;
		...
		params->dsi.HS_PRPR  = 3;//min 40ns+4*UI; max 85ns+6UI 
		// Bit rate calculation
#ifdef CMD_MODE_USED
		params->dsi.pll_div1=1; 	// div1=0,1,2,3;div1_real=1,2,4,4
		params->dsi.pll_div2=1;			// div2=0~15: fout=fvo/(2*div2)
		params->dsi.fbk_div =14;		// fref=26MHz, fvco=fref*(fbk_div+1)*fbk_real/(div1_real*div2_real)
...
{  
	...
	lcm_get_id_pin();
	mt_set_gpio_mode(76, 0); 
	mt_set_gpio_dir(76, 1);
    ...
    if(g_lcd_id_value3 == BYD)
	    ...
	    push_table(lcm_vdo_truly_initialization_setting, sizeof(lcm_vdo_truly_initialization_setting) / sizeof(struct LCM_setting_table), 1);
    //DMS_LCD("lcm_init---------------------------------led_on = %d\r\n", OPPO_LED_ON);	
    DMS_LCD("===============>lcm_init end,g_lcd_id_value3=%d\n",g_lcd_id_value3);
#else
		if(g_lcd_id_value3 == BYD)
			push_table(lcm_cmd_byd_initialization_setting, sizeof(lcm_cmd_byd_initialization_setting) / sizeof(struct LCM_setting_table), 1);
		else if(g_lcd_id_value3 == TRULY)
...
static void lcm_suspend(void)
{
    ...
    DMS_LCD("===============>lcm_suspend,g_lcd_id_value3=%d\n",g_lcd_id_value3);
		...
		mt_set_gpio_dir(74,1);
		mt_set_gpio_out(74,1);
#endif
		mt_set_gpio_mode(76, 0); 
		mt_set_gpio_dir(76,1);
    ...
    if(!istesuspend)
    {
	    ...
	    push_table(lcm_vdo_sleep_mode_in_setting, sizeof(lcm_vdo_sleep_mode_in_setting) / sizeof(struct LCM_setting_table), 1);
#else	
        push_table(lcm_cmd_sleep_mode_in_setting, sizeof(lcm_cmd_sleep_mode_in_setting) / sizeof(struct LCM_setting_table), 1);
#endif      
...
{
    DMS_LCD("lcm_resume start: g_lcd_id_value3 = %d\n", g_lcd_id_value3);
		mt_set_gpio_mode(76, 0); 
		mt_set_gpio_dir(76, 1);
		mt_set_gpio_out(76,1);
    SET_RESET_PIN(0);
    ...
    DMS_LCD("lcm_resume end: g_lcd_id_value3 = %d\n", g_lcd_id_value3);
...
static unsigned char buffer_hitachi_d0[10] ={0xa9,0x03,0xbd,0xa5,0x0c,0x72,0x20,0x10,0x01,0x0};
   static unsigned char buffer_B0_s[2] ={0x01,0x0b};
   static 	unsigned char buffer_B9_s[3]={0xff,0x83,0x0};
   static 	unsigned char buffer_B4_s[5]={0x2,0xc,0x0,0x0,0x00};	
   static 	unsigned char buffer_B6_s[2]={0x2c,0x2c};	
