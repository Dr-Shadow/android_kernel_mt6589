...
static struct i2c_board_info __initdata i2c_l3g4200d={ I2C_BOARD_INFO(L3G4200D_DEV_NAME, (0xD4>>1))};
...
static void L3G4200D_dumpReg(struct i2c_client *client)
{
	...
	for(i = 0; i < 25 ; i++)
	{
		...
		regdata = i2c_smbus_read_byte_data(client, addr);
		
		HWM_LOG("L3GD20 Reg addr = 0x%x , regdata = 0x%x\n", addr,regdata);
...
static int L3G4200D_SetPowerMode(struct i2c_client *client, bool enable)
{
	...
	printk("%s, write to L3G4200D_CTL_REG1 is  = 0x%x\n", __func__, databuf[0]);
...
static int L3G4200D_SetDataResolution(struct i2c_client *client, u8 dataResolution)
{
	...
	databuf[0] = i2c_smbus_read_byte_data(client, L3G4200D_CTL_REG4);
	if( databuf[0] < 0)
	{
...
static int L3G4200D_SetSampleRate(struct i2c_client *client, u8 sample_rate)
{
	...
	databuf[0] = i2c_smbus_read_byte_data(client, L3G4200D_CTL_REG1);
	if(databuf < 0)
	{
...
static int L3G4200D_ReadGyroData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	else
	{
		...
		data[L3G4200D_AXIS_X] = data[L3G4200D_AXIS_X] *  SENSITIVITY_FS_2000DPS ;
		data[L3G4200D_AXIS_Y] = data[L3G4200D_AXIS_Y] *  SENSITIVITY_FS_2000DPS ;
		data[L3G4200D_AXIS_Z] = data[L3G4200D_AXIS_Z] *  SENSITIVITY_FS_2000DPS ; 
	}

	data[L3G4200D_AXIS_X] += obj->cali_sw[L3G4200D_AXIS_X];
	data[L3G4200D_AXIS_Y] += obj->cali_sw[L3G4200D_AXIS_Y];
	data[L3G4200D_AXIS_Z] += obj->cali_sw[L3G4200D_AXIS_Z];
...
int L3G4200D_OpenGyro_preview(void)
{
	...
	if(sensor_power == true)				// gyro has been opened before carmer preview
	{
		gyro_static_befor_preview = true;
		printk("%s: Gyro has been opened before carmer preview. \n", __func__);
	}
	else
	{
		gyro_static_befor_preview = false;
		printk("%s: Gyro is not running before carmer preview, so open it. \n", __func__);		
	}
	
	nRet = L3G4200D_SetPowerMode(l3g4200d_i2c_client, true);
	if (nRet != 0)
	{
		printk("Open gyro failed \r\n");
		return -1;
	}
	return 0;
}

//  for carmer preview close gyro
int L3G4200D_CloseGyro_preview(void)
{
	int nRet = -1;

	if(gyro_static_befor_preview == true)	// gyro has been opened before carmer preview
	{
		printk("%s: Gyro has been opened before carmer preview, so don't close it!\n",__func__);
	}
	else
	{
		printk("%s: Gyro is not running before carmer preview, so close it!\n",__func__);		

		nRet = L3G4200D_SetPowerMode(l3g4200d_i2c_client, false);
		if (nRet != 0)
		{
			printk("Close gyro failed \r\n");
			return -1;
		}
	}
	return 0;
...
// for carmer preview read gyro data
int L3G4200D_ReadGyroData_preview(int *buf)
{
	char databuf[6];	
	int data[3];
	struct l3g4200d_i2c_data *obj = i2c_get_clientdata(l3g4200d_i2c_client);  

	if(hwmsen_read_block(l3g4200d_i2c_client, AUTO_INCREMENT |L3G4200D_REG_GYRO_XL, databuf, 6))
	{
		GYRO_ERR("L3G4200D read gyroscope data	error\n");
		return -2;
	}
	else
	{
		...
		data[L3G4200D_AXIS_X] = data[L3G4200D_AXIS_X] * SENSITIVITY_FS_2000DPS * 1000 / DEGREE_TO_RAD;
		data[L3G4200D_AXIS_Y] = data[L3G4200D_AXIS_Y] * SENSITIVITY_FS_2000DPS * 1000 / DEGREE_TO_RAD;
		data[L3G4200D_AXIS_Z] = data[L3G4200D_AXIS_Z] * SENSITIVITY_FS_2000DPS * 1000 / DEGREE_TO_RAD; 
	}
...
static int L3G4200D_SelfTest(struct i2c_client *client)
{
	...
	data = i2c_smbus_read_byte_data(client, L3G4200D_STATUS_REG);
	...
	data = i2c_smbus_read_byte_data(client, L3G4200D_CTL_REG4);
	data = data | 0x02;
...
#ifdef MOTION_SENSITIVE_GYRO

	// sensors not suspend( motion sensing ), Gyro works normally 
	if( sensor_suspend == 0 )	
	{
		atomic_set(&obj->sensor_suspend_flag, 1);
	
		printk("%s: Need motion sensing , Gyro not suspend and works normally !\n", __func__);

		return ;		
	}
	else
	{
		printk("%s, motion sensing switch closed , Gyro will suspend !\n", __func__);
	}
...
static void l3g4200d_late_resume(struct early_suspend *h)
{
	...
	{
		printk("%s, motion sensing switch closed , Gyro will resume !\n", __func__);
	}
...
static int l3g4200d_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	printk("L3g4200d addr is 0x%x!\n",client->addr);

	obj->hw = get_cust_gyro_hw_l3gd20();
	...
	err = l3g4200d_init_client(new_client, true);
	if(err)
	{
		printk("l3gd20_init_client failed, not l3gd20 or bad chip!\n");	
...
{
	struct gyro_hw *hw = get_cust_gyro_hw_l3gd20();
	GYRO_FUN();
