...
static struct i2c_board_info __initdata i2c_stk31xx={ I2C_BOARD_INFO(STK31XX_DEV_NAME, (0x90>>1))};
...
void recovery_mode(int ps_data);
...
{
    return get_cust_alsps_hw_stk31xx();
}
...
static struct workqueue_struct *  ps_workqueue=NULL;
...
{	
    ...
    val = atoi(hex);
    set_reg = reg;
    ret = i2c_smbus_write_byte_data(stk31xx_i2c_client, reg, val);
    if (ret < 0)
...
static ssize_t read_prox_reg_read(struct file *file, char __user *buf, size_t count, loff_t *pos)
{
    ...
    for (i = 0x01; i <= 0x0c; i += 0x01){
        //i2c_master_send(stk31xx_i2c_client, &i, 1);
        //i2c_master_recv(stk31xx_i2c_client, &val, 1);
        val = i2c_smbus_read_byte_data(stk31xx_i2c_client, i);
...
static ssize_t read_prox_reg_write(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
    ...
    reg = atoi(tmp);
    val = i2c_smbus_read_byte_data(stk31xx_i2c_client, reg);
    if (val < 0)
...
static ssize_t read_ps_switch_status(struct file *file, char __user *buf, size_t count, loff_t *pos)
{
    ...
    printk("stk31xx  show_ps_switch ,  g_ps_switch_off:%d \n", g_ps_switch_off);
    p += sprintf(p, "%d\n", g_ps_switch_off);	
...
static ssize_t set_ps_switch(struct file *file, char __user *buf, size_t count, loff_t *pos)
{
    ...
    sscanf(tmp, "%d", &val);
    APS_LOG("stk31xx set_ps_switch %d\n", val);
...
ALS_DT2  :  LSB	*/
    ...
    else if(1 != (ret = stk31xx_master_recv(client, obj->addr.als_dat2, (char*)&buf[0], 1)))
    {
...
int stk31xx_read_ps(struct i2c_client *client, u8 *data)
{
    ...
    //recovery_mode(ret);
#ifdef VENDOR_EDIT
    //zhanhua.li@Prd.BasicDrv.Sensor,2013/04/03,fix ps=0 bug
    //it maybe the device bug,ignore the ps value
    if(ret==0 ){    
        printk(KERN_ERR"%s ps bias:ret=%d,last=%d\n",__func__,ret,last_ps);
        ret = last_ps;        
    }
    last_ps = ret;
#endif
...
/*----------------------------------------------------------------------------*/
static int stk31xx_dump_all_reg(struct i2c_client *client)
{
    u8 val = 0;
    //u8 reg = 0x01;
    u8 i;
...
#else
    err = stk31xx_write_als(client, cur);
#endif
...
/*----------------------------------------------------------------------------*/
static void stk31xx_set_ps_threshold(struct i2c_client *client, int low_threshold, int high_threshold);
...
static int stk31xx_enable_ps(struct i2c_client *client, int enable)
{
    ...
    if(enable)
    {
        ...
        ps_invalid_flags = 0;
        ps_eint_mode = 0;
        printk("%s  PS   start  work\n",__func__);	
#endif
        atomic_set(&obj->ps_deb_on, 1);
        atomic_set(&obj->ps_deb_end, jiffies+atomic_read(&obj->ps_debounce)/(1000/HZ));
        set_bit(STK_BIT_PS,  &obj->pending_intr);
        stk31xx_set_ps_threshold(obj->client,g_ps_low_threshold,g_ps_high_threshold);

        {
            ...
            stk31xx_clear_intr(client, ALSPS_STATUS, 0x0); // clear int when enable
...
static int stk31xx_check_intr(struct i2c_client *client, u8 *status) 
{
    ...
    err = i2c_smbus_read_byte_data(stk31xx_i2c_client, obj->addr.status);
    if (err < 0)
    {
    ...
    *status = err;
    printk("stk31xx_check_intr: read status reg: 0x%x\n", *status);
...
static int stk31xx_clear_intr(struct i2c_client *client , u8 status, u8 disable_flag) 
{
    ...
    printk("stk31xx_clear_intr: set status reg = 0x%x to value = 0x%x\n",obj->addr.status, status);
    ...
    if (ret < 0)
    {
        printk(KERN_ERR"stk31xx clear interrupt fail !!!\n");
        return -EFAULT;
    }
#endif
    return 0;
...
static int stk31xx_set_als_int_thd(struct i2c_client *client, u16 als_data_reg) 
{
    ...
    if (als_thd_l <0)
        als_thd_l = 0;
    printk("stk31xx_set_als_int_thd:als_thd_h:%d,als_thd_l:%d\n", als_thd_h, als_thd_l);	
...
/*----------------------------------------------------------------------------*/
int stk31xx_set_ps_mode(struct stk31xx_priv *obj, int ps_eint)
{
    int ret = 0;
    u8 val;
    ...
    if (ps_eint){
        ...
        ret = stk31xx_write_ps(obj->client, val);//61
        //msleep(10);
        val = (tmp | INT_PS) & ~SD_PS; // open ps with eint mode
        ret |= stk31xx_write_ps(obj->client, val);//62
    } else {
#ifndef VENDOR_EDIT
        val = tmp | SD_PS; // shut down ps
        ret = stk31xx_write_ps(obj->client, val);
        //msleep(10);
        val = (tmp & ~INT_PS) & ~SD_PS; // open ps with polling mode
        ret |= stk31xx_write_ps(obj->client, val);
#else
        val = (tmp & ~INT_PS) & ~SD_PS;
        ret |= stk31xx_write_ps(obj->client, val);
#endif
    }
...
void recovery_mode(int ps_data){
    if( ps_eint_mode == 1 && !g_in_interrupt ){
        //int mode
        int err;
        hwm_sensor_data sensor_data;
        if( ps_data >= g_ps_high_threshold ) {

            printk("%s report near \n",__func__);
            distance = 0;

            // err = hwmsen_get_interrupt_data(ID_PROXIMITY, &sensor_data);

        } else if ( ps_data < g_ps_low_threshold){
            //error ,back to polling mode
            printk("stk31xx %s back to polling mode ps=%d,low_thd=%d\n",
                    __func__,ps_data,g_ps_low_threshold);
            stk31xx_set_ps_mode(stk31xx_obj , 0);
            sensor_data.values[0] = 1;
            sensor_data.value_divide = 1;
            sensor_data.status = SENSOR_STATUS_ACCURACY_MEDIUM;
            err = hwmsen_get_interrupt_data(ID_PROXIMITY, &sensor_data);
            distance = 1;
            stk31xx_dump_all_reg(stk31xx_obj->client);
        } else {
            printk ( "%s do nothing\n",__func__);
        }
...
void stk31xx_eint_func(void)
{
    ...
    queue_work(ps_workqueue, &obj->eint_work );
...
static void stk31xx_eint_work(struct work_struct *work)
{
    ...
    if((g_is_far &&  ( (obj->ps) < g_ps_high_threshold ))  ||
            ((g_is_far==0) && ( (obj->ps) >= g_ps_high_threshold )))
    {
        printk("%s : ignore interrupt !!!\n", __func__);
        goto exit;
    }
    ...
    sensor_data.status = SENSOR_STATUS_ACCURACY_MEDIUM;
    err = hwmsen_get_interrupt_data(ID_PROXIMITY, &sensor_data);
    printk("%s to:%s distance=%d ps=%d  set ps threshold  low=%d  high=%d\n",__func__,g_is_far?"far":"near",
            distance,obj->ps,g_ps_low_threshold, g_ps_high_threshold);
    stk31xx_set_ps_threshold(obj->client, g_ps_low_threshold, g_ps_high_threshold);
...
//    if(err = stk31xx_dump_all_reg(client))
//    {
...
static ssize_t stk31xx_show_status(struct device_driver *ddri, char *buf)
{
    ...
    if(stk31xx_obj->hw)
    {
        len += snprintf(buf+len, PAGE_SIZE-len, "CUST: %d, (%d %d) (%02X %02X) (%02X %02X %02X) (%02X %02X %02X)\n", 
                ...
                stk31xx_obj->addr.status,stk31xx_obj->addr.als_cmd, stk31xx_obj->addr.als_dat2, stk31xx_obj->addr.als_dat1,
...
static int stk31xx_get_ps_value(struct stk31xx_priv *obj, u16 ps)
{
    ...
    if (val == 0) //when ps value is near, to detect environment light intensity,to ensure indeed getting close
    {
        /*	ALS_DT1  :  MSB
ALS_DT2  :  LSB	*/
        buf[1] = i2c_smbus_read_byte_data(stk31xx_i2c_client, ALS_DT1);
        buf[0] = i2c_smbus_read_byte_data(stk31xx_i2c_client, ALS_DT2);
        ...
        unsigned long arg)
{
    ...
    switch (cmd)
    {
        case ALSPS_SET_PS_MODE:
            ...
            if (g_stk31xx_ptr->fasync != NULL)
            {
                APS_LOG("kill a SIGIO when %s", __func__);
                kill_fasync(&g_stk31xx_ptr->fasync, SIGIO, POLL_IN);
            }
        ...
        case ALSPS_GET_PS_DATA:
            ...
            if ((distance == 0) && (ps_eint_mode == 0)){
                stk31xx_set_ps_mode(obj, 1); //change ps mode to eint
                //stk31xx_set_ps_threshold(obj->client, 0, 0);//just for test
            }
...
#endif
            dat = obj->ps;
            recovery_mode(obj->ps);
            if (dat == 0)
                err = stk31xx_dump_all_reg(obj->client);
        ...
        case ALSPS_SET_ALS_MODE:
            ...
            if(enable)
            {
                if (suspend_status == 1){
                    ...
                    stk31xx_power(stk31xx_obj->hw, 1);
                    ...
                    atomic_set(&stk31xx_obj->ps_suspend, 0);
                    suspend_status = 0;
                    atomic_set(&stk31xx_obj->als_suspend, 0);
                }
        ...
        case ALSPS_GET_ALS_DATA: 
            ...
            break;
#else
            temp_level = stk31xx_get_als_value(obj, obj->als);
        ...
        case ALSPS_SET_PS_THRESHOLD: //0x21 lycan add for 
            ...
            stk31xx_set_ps_threshold(obj->client,
                    set_ps_thd_para.low_threshold, 
                    set_ps_thd_para.high_threshold);
            break;
...
static int stk31xx_fasync(int fd, struct file * file, int on)
{
    APS_LOG("%s on : %d\n", __func__, on);
    return fasync_helper(fd, file, on, &g_stk31xx_ptr->fasync);
}
/*----------------------------------------------------------------------------*/
...
static int stk31xx_i2c_suspend(struct i2c_client *client, pm_message_t msg) 
{
    ...
    {   
        ...
        atomic_set(&stk31xx_obj->als_suspend, 1);

        if (test_bit(STK_BIT_PS,  &stk31xx_obj->enable))
        {
        ...
        if(test_bit(STK_BIT_ALS,  &stk31xx_obj->enable)){
        ...
        suspend_status = 1;
        atomic_set(&stk31xx_obj->ps_suspend, 1);
        ...
        stk31xx_power(stk31xx_obj->hw, 0);
    }
    up(&ps_suspend_sem);
...
static int stk31xx_i2c_resume(struct i2c_client *client)
{
    ...
    if ((test_bit(STK_BIT_PS,  &stk31xx_obj->enable)) || (suspend_status == 0)){
    ...
    if (err < 0)
    {
        printk(KERN_ERR"stk31xx resume clear interrupt fail !!!\n");
        up(&ps_suspend_sem);
        return err;
    }
...
{   /*early_suspend is only applied for ALS*/
    ...
    err = stk31xx_i2c_suspend(stk31xx_obj->client, msg);
    //up(&ps_suspend_sem);
#endif
}
/*----------------------------------------------------------------------------*/
...
#else
    err = stk31xx_i2c_resume(stk31xx_obj->client);
    //up(&ps_suspend_sem);
#endif
}
            ...
            else
            {
                ...
                if(value)
                {
                    ...
                    if (suspend_status != 0)
                    {
                        ...
                        stk31xx_power(stk31xx_obj->hw, 1);
                        if((err = stk31xx_init_client(obj->client)))
                            APS_ERR("initialize client fail!!\n");
...
static void stk31xx_set_ps_threshold(struct i2c_client *client, int low_threshold, int high_threshold)
{
...
#ifndef VENDOR_EDIT
    res = stk31xx_write_ps_high_thd(client, databuf[1]);
    res = stk31xx_write_ps_low_thd(client, databuf[0]);
#else
    ...
    if(alsps_dbg_flag )
        printk("%s realy set low=%d,high=%d\n",__func__,databuf[0],databuf[1]);
#endif
    if (res < 0)
    {
...
EXIT_ERR:
    APS_ERR("stk31xx_enable_ps fail\n");
    return;
}
/**zhye**add**end**/
#endif/*VENDOR_EDIT*/
...
static int stk31xx_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
#ifndef VENDOR_EDIT
    INIT_DELAYED_WORK(&stk31xx_ps_work, stk31xx_ps_adjust_delayed_work);
#endif
    INIT_WORK(&obj->eint_work, stk31xx_eint_work);
    obj->client = client;
    ...
    //atomic_set(&obj->ps_cmd_val, 0x71);
    gain_setting = 0x01;
    ...
    ps_workqueue = create_singlethread_workqueue("stk31xx-ps");
    if ( ps_workqueue == NULL ){
        printk("%s create ps_workqueue fail\n", __func__);
        goto exit_create_attr_failed;
    }
    printk("%s: OK\n", __func__);
    return 0;
