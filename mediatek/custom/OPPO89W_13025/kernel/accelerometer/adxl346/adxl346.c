...
//#include <mach/mt_devs.h>/*OPPO 2013-01-21 zhye add for device init*/
...
#else
    dat[obj->cvt.map[ADXL346_AXIS_X]] = obj->cvt.sign[ADXL346_AXIS_X]*(obj->cali_sw[ADXL346_AXIS_X]);
    dat[obj->cvt.map[ADXL346_AXIS_Y]] = obj->cvt.sign[ADXL346_AXIS_Y]*(obj->cali_sw[ADXL346_AXIS_Y]);
    dat[obj->cvt.map[ADXL346_AXIS_Z]] = obj->cvt.sign[ADXL346_AXIS_Z]*(obj->cali_sw[ADXL346_AXIS_Z]);    
#endif
...
#else
		obj->cali_sw[ADXL346_AXIS_X] = dat[0];
		obj->cali_sw[ADXL346_AXIS_Y] = dat[1];
		obj->cali_sw[ADXL346_AXIS_Z] = dat[2];
#endif 
...
static int ADXL346_ReadSensorData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	else
	{
		...
		acc[ADXL346_AXIS_Z] = acc[ADXL346_AXIS_Z] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;		
			...
			{
				acc[ADXL346_AXIS_X] += obj->cali_sw[ADXL346_AXIS_X];
...
/*----------------------------------------------------------------------------*/
static ssize_t show_direction_value(struct device_driver *ddri, char *buf, size_t count)
{
	ssize_t res;
	struct adxl346_i2c_data *obj = obj_i2c_data;
	if (obj == NULL)
	...
	res = snprintf(buf, PAGE_SIZE, "current direction: %d \n", obj->hw->direction);     
		...
		void* buff_out, int size_out, int* actualout)
{
	...
	{
		if (command != SENSOR_ENABLE)
			return 0;
		value = *(int*)buff_in;
		if (value)
			atomic_set(&priv->enable_before_resume, 1);
		else
			sensor_power = false;

		GSE_LOG("Enable sensor before power on with value %d.", value);
		return 0;
	}
...
static int calculate_gsensor_cali_data(struct i2c_client *client, s16 data[3])
{
	...
	{		
		...
		if(res = ADXL346_ReadSensorData(client, buff,ADXL346_BUFSIZE))
		{        
			GSE_ERR("I2C error: ret value=%d", res);
			return -EIO;
		}
	...
	//bma250_store_offset_to_file(obj->cali_sw);
	printk("calculate_gsensor_cali_data (0,0,9800)-(%d, %d, %d)=(%d, %d, %d) \n", 
			average_offset[0],average_offset[1],average_offset[2],
...
static void adxl346_early_suspend(struct early_suspend *h) 
{
	...
	if (!sensor_suspend){
	    //need_power = false;
		printk("%s  ps is working, not suspend,ss=%d,power=%d\n",__func__,sensor_suspend,need_power);
		return;
	}
	
	need_power= true;
	printk("%s power off ss=%d,power=%d\n",__func__,sensor_suspend,need_power);
	atomic_set(&obj->suspend, 1); 
	if((err = ADXL346_SetPowerMode(obj->client, false)))
	{
		GSE_ERR("write power control fail!!\n");
...
static void adxl346_late_resume(struct early_suspend *h)
{
	...
	if (!sensor_suspend && !need_power){
		printk("%s  ps is working, not resume ss=%d,power=%d\n",__func__,sensor_suspend,need_power);
		return;
	}
	need_power = false;
...
static int adxl346_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	struct acc_hw *hw_probe = get_cust_acc_hw();
	int err = 0;
	GSE_FUN();
...
//LiuPing@Prd.BasicDrv.Sensor, add 2012/12/03 ,because of the same i2c addr as mma8453q,set other addr to register driver successful.
	...
	int i;
	i=3;
	while(err = adxl346_init_client(new_client, 1))
	{
		GSEN_DBG("adxl346_init_client err=%d NO=%d \n",err,i);
		msleep(100);
		i--;
		if(i == 0)
		   goto exit_init_failed;
...
static int __init adxl346_init(void)
{
	...
	#else
	i2c_register_board_info(2, &i2c_adxl346, 1);
	#endif
