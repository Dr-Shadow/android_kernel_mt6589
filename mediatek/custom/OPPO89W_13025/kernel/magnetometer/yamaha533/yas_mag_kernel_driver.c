...
#define DEBUG_RW 0
/*----------------------------------------------------------------------------*/
#define YAMAHA_DEV_NAME         "yamaha533"
#define DRIVER_VERSION          "1.0.1"
/*----------------------------------------------------------------------------*/
#define YAMAHA533_AXIS_X            0
#define YAMAHA533_AXIS_Y            1
#define YAMAHA533_AXIS_Z            2
#define YAMAHA533_AXES_NUM          3
...
/*----------------------------------------------------------------------------*/
/*----------------------------------------------------------------------------*/
static const struct i2c_device_id yamaha533_i2c_id[] = {{YAMAHA_DEV_NAME,0},{}};
static struct i2c_board_info __initdata i2c_yamaha533={ I2C_BOARD_INFO("yamaha533", (0x5c>>1))};
/*the adapter id will be available in customization*/
//static unsigned short yamaha533_force[] = {0x00, YAS_MAG_I2C_SLAVEADDR, I2C_CLIENT_END, I2C_CLIENT_END};
//static const unsigned short *const yamaha533_forces[] = { yamaha533_force, NULL };
//static struct i2c_client_address_data yamaha533_addr_data = { .forces = yamaha533_forces,};
/*----------------------------------------------------------------------------*/
static int yamaha533_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id); 
static int yamaha533_i2c_remove(struct i2c_client *client);
static int yamaha533_i2c_detect(struct i2c_client *client, int kind, struct i2c_board_info *info);

static struct platform_driver yamaha_sensor_driver;
struct yas_mag_data g_mag_data;
int mag_accuracy = 0;
        ...
        char *buf)
{
    ...
    geomagnetic_multi_unlock();
printk("%s  data->distortion[0]=%d,data->distortion[1]=%d,data->distortion[2]=%d\n",__func__,data->distortion[0],data->distortion[1],data->distortion[2]);
    return rt;
}
...
printk("%s Enter !\n",__func__);
    ...
    geomagnetic_multi_unlock();
printk( "%s offset.hard_offset[0]=%d,offset.hard_offset[1]=%d,offset.hard_offset[2]=%d,offset.calib_offset.v[0]=%d,offset.calib_offset.v[1]=%d,offset.calib_offset.v[2]=%d,accuracy=%d\n",
	__func__,offset.hard_offset[0],
...
geomagnetic_work(struct yas_mag_data *magdata)
{
    ...
    if (rt >= 0) {
        accuracy = atomic_read(&data->last_status);
		mag_accuracy = atomic_read(&data->last_status);
...
printk("%s 3\n",__func__);
		...
		for (i = 0; i < 3; i++)
			g_mag_data.xyz.v[i] = magdata->xyz.v[i];
...
printk("%s 4\n",__func__);
		...
		for (i = 0; i < 3; i++)
			g_mag_data.raw.v[i] = magdata->raw.v[i];
...
/*----------------------------------------------------------------------------*/
static int yamaha533_open(struct inode *inode, struct file *file)
{    
	int ret = -1;
...
/*----------------------------------------------------------------------------*/
static int yamaha533_release(struct inode *inode, struct file *file)
{	
...
/*----------------------------------------------------------------------------*/
//static long yamaha533_ioctl(struct inode *inode, struct file *file, unsigned int cmd,unsigned long arg)
	static long yamaha533_unlocked_ioctl(struct file *file, unsigned int cmd,unsigned long arg)

{
	...
	struct geomagnetic_data *obj = i2c_get_clientdata(this_client);
...
//	MSE_FUN(f);
	...
	switch (cmd)
	{
		...
		case MSENSOR_IOCTL_READ_FACTORY_SENSORDATA:
			...
			osensor_data->values[0] = atomic_read(&obj->last_data[0]) * CONVERT_O;
			osensor_data->values[1] = atomic_read(&obj->last_data[1]) * CONVERT_O;
			osensor_data->values[2] = atomic_read(&obj->last_data[2]) * CONVERT_O;
			osensor_data->status = atomic_read(&obj->last_status);
...
/*----------------------------------------------------------------------------*/
static struct file_operations yamaha533_fops = {
	.owner = THIS_MODULE,
	.open = yamaha533_open,
	.release = yamaha533_release,
	.unlocked_ioctl = yamaha533_unlocked_ioctl,
};
/*----------------------------------------------------------------------------*/
static struct miscdevice yamaha533_device = {
    .minor = MISC_DYNAMIC_MINOR,
    .name = "msensor",
    .fops = &yamaha533_fops,
};
/*----------------------------------------------------------------------------*/
int yamaha533_operate(void* self, uint32_t command, void* buff_in, int size_in,
...
static struct driver_attribute *yamaha533_attr_list[] = {
    &driver_attr_daemon,
    &driver_attr_daemon2,
	
};

static int yamaha533_create_attr(struct device_driver *driver) 
{
	int idx, err = 0;
	int num = (int)(sizeof(yamaha533_attr_list)/sizeof(yamaha533_attr_list[0]));
	if (driver == NULL)
	{
		return -EINVAL;
	}
	...
	{
		if((err = driver_create_file(driver, yamaha533_attr_list[idx])))
		{            
			MSE_ERR("driver_create_file (%s) = %d\n", yamaha533_attr_list[idx]->attr.name, err);
			break;
		}
	}    
	return err;
...
static int yamaha533_delete_attr(struct device_driver *driver)
{
	int idx ,err = 0;
	int num = (int)(sizeof(yamaha533_attr_list)/sizeof(yamaha533_attr_list[0]));
	...
	for(idx = 0; idx < num; idx++)
	{
		driver_remove_file(driver, yamaha533_attr_list[idx]);
	}
...
static int yamaha533_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if(yamaha533_create_attr(&yamaha_sensor_driver.driver))
	{
		printk("yamaha533 create attribute err\n");
		goto  err;
	}
	...
	if(rt = misc_register(&yamaha533_device))
	{
		MSE_ERR("yamaha533_device register failed\n");
		goto err;
	}    
	sobj_m.self = data;
    sobj_m.polling = 1;
    sobj_m.sensor_operate = yamaha533_operate;
...
/*----------------------------------------------------------------------------*/
static struct i2c_driver yamaha533_i2c_driver = {
    .driver = {
//        .owner = THIS_MODULE, 
        .name  = YAMAHA_DEV_NAME,
    },
	.probe      = yamaha533_i2c_probe,
	.remove     = yamaha533_i2c_remove,
	.detect     = yamaha533_i2c_detect,
	.suspend    = geomagnetic_suspend,
	.resume     = geomagnetic_resume,

	.id_table = yamaha533_i2c_id,
//	.address_data = &yamaha533_addr_data,
};
/*----------------------------------------------------------------------------*/
...
static int yamaha533_i2c_detect(struct i2c_client *client, int kind, struct i2c_board_info *info) 
{    
	strcpy(info->type, YAMAHA_DEV_NAME);
	return 0;
}
...
static int yamaha533_i2c_remove(struct i2c_client *client)
{
	...
	if(data != NULL)
	{
		...
		if(err = yamaha533_delete_attr(&yamaha_sensor_driver.driver))
	    {
		   printk("yamaha533_delete_attr fail: %d\n", err);
	    }
	...
	misc_deregister(&yamaha533_device);    
	return 0;
}
/*----------------------------------------------------------------------------*/
static int yamaha_probe(struct platform_device *pdev) 
{
	...
	//yamaha533_power(hw, 1);    
	//yamaha533_force[0] = hw->i2c_num;
	if(i2c_add_driver(&yamaha533_i2c_driver))
	{
...
static int yamaha_remove(struct platform_device *pdev)
{
	...
	MSE_FUN();    
	//yamaha533_power(hw, 0);    
	atomic_set(&dev_open_count, 0);  
	i2c_del_driver(&yamaha533_i2c_driver);
	return 0;
}
/*----------------------------------------------------------------------------*/
...
static int __init yamaha533_init(void)
{
	...
	i2c_register_board_info(2, &i2c_yamaha533, 1);
