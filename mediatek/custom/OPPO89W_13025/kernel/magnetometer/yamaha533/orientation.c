...
/*----------------------------------------------------------------------------*/
extern struct yas_mag_data g_mag_data;
extern int mag_accuracy;
int yamaha_orientation_operate(void* self, uint32_t command, void* buff_in, int size_in,
...
//	unsigned long flags;
	...
	//printk("yamaha_orientation_operate!\n");
	switch (command)
	{
		case SENSOR_DELAY:
		...
		case SENSOR_GET_DATA:
			...
			#else
				osensor_data->values[0] = g_mag_data.raw.v[0];
				osensor_data->values[1] = g_mag_data.raw.v[1];
				osensor_data->values[2] = g_mag_data.raw.v[2];
				osensor_data->status = mag_accuracy;
			#endif
				printk(" \n%s Sidney debug \n",__func__);
				printk("osensor_data->values[0] is %d \n",osensor_data->values[0]);
				printk("osensor_data->values[1] is %d \n",osensor_data->values[1]);
				printk("osensor_data->values[2] is %d \n\n",osensor_data->values[2]);
...
//    int misc_registered = 0;
	...
	sobj.self = data;
    sobj.polling = 1;
    sobj.sensor_operate = yamaha_orientation_operate;
