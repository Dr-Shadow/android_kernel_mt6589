...
#define MMCDBG(format, ...)	printk( "mmc328x " format "\n", ## __VA_ARGS__)
#else
#define MMCDBG(format, ...)
#endif

#if MMC328x_DEBUG_FUNC
#define MMCFUNC(func) printk( "mmc328x " func " is called\n")
#else
#define MMCFUNC(func)
#endif
...
static ssize_t show_autotest_testID(struct device_driver *ddri, char *buf)
{
	...
	/*check device ID*/
/*	data[0] = MMC328X_REG_ID0;
	if (I2C_RxData(data, 1) < 0)
       	{
		goto READ_ID_FAIL;
	}
	id[0] = data[0];
	data[0] = MMC328X_REG_ID1;
	if (I2C_RxData(data, 1) < 0)
       	{
		goto READ_ID_FAIL;
	}
...
*/
	...
	/*Enable*/
	data[0] = MMC328x_CTRL_TM;
	data[1] = MMC328x_REG_CTRL;
	if (I2C_TxData(data, 2) < 0)
		goto RESET_DEV_FAIL;
	msleep(MMC328x_DELAY_SET);
...
static ssize_t show_autotest_magnetclose(struct device_driver *ddri, char *buf)
{
	...
	for (i = 0; i < 50; i ++)
	{
		...
		{
			printk(KERN_ERR"Auto Test:close read data fail");
			break;
		}

		/*Data availble??*/
		if ((8191 == data_magnet_close[0])
				|| (8191 == data_magnet_close[1])
				|| (8191 == data_magnet_close[2]))
		{
			printk(KERN_ERR"Auto Test:close read data fail");
			break;
		}
...
static ssize_t show_autotest_magnetleave(struct device_driver *ddri, char *buf)
{
	int i ;
	unsigned char data[2];
#if 0//Shaoyu.Huang@BasicDrv.Sensor delete to simplify magnetometer auto test
	for (i = 0; i < 50; i ++)
	{
		data[0] = MMC328x_REG_CTRL;
		data[1] = MMC328x_CTRL_RM;
		...
		if (I2C_TxData(data, 2) < 0)
			break;
		msleep(MMC328x_DELAY_SET);
		/*----all above to reset device to initial status----*/

		if (ECS_ReadXYZData(data_magnet_leave, 3))
		{
			printk(KERN_ERR"Auto Test:leave read data fail");
			break;
		}
		printk(KERN_ERR"%d, %d, %d", data_magnet_leave[0], data_magnet_leave[1], 
				data_magnet_leave[2]);
		if ((8191 == data_magnet_leave[0])
				|| (8191 == data_magnet_leave[1])
				|| (8191 == data_magnet_leave[2]))
		{
			printk(KERN_ERR"Auto Test:leave read data fail");
			break;
		}		
		if ((abs(data_magnet_leave[0] - data_initial[0]) <= 50)
				&& (abs(data_magnet_leave[1] - data_initial[1]) <= 50)
				&& (abs(data_magnet_leave[2] - data_initial[2]) <= 50))
			return 0;
		msleep(100);
...
static ssize_t show_autotest_get_ic_modle(struct device_driver *ddri, char *buf)	
{
	buf="mmc328x";
...
//mingqiang.guo add for muti msensor
struct mag_hw *get_cust_mag_hw(void) 
{
    return get_cust_mag_hw_mmc328xma();
}
...
static int mmc328x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	struct mag_hw *hw_probe = get_cust_mag_hw();
...
{
	MSEN_DBG("%s\n",__func__);
	i2c_register_board_info(2, &i2c_mmc328xma, 1);
