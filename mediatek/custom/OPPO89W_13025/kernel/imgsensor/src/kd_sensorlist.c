...
/************************************************************************************/
/*OPPO zhangkw modfy start 2013-01-29*/
#if 0
...
#else
#define SUPPORT_I2C_BUS_NUM1        1     //for main 
#define SUPPORT_I2C_BUS_NUM2        3     //for sub
...
static struct i2c_board_info __initdata i2c_devs2={I2C_BOARD_INFO(CAMERA_HW_DRVNAME2, 0xfe>>1)};
#endif
/*OPPO zhangkw modfy end 2013-01-29*/
/************************************************************************************/
...
int iReadRegI2C(u8 *a_pSendData , u16 a_sizeSendData, u8 * a_pRecvData, u16 a_sizeRecvData, u16 i2cId)
{
    ...
    if(gI2CBusNum == SUPPORT_I2C_BUS_NUM1) {
		...
		{
			memcpy(g_pI2CDMAVirtualAddr, a_pSendData, a_sizeSendData);
			g_pstI2Cclient->ext_flag |= I2C_DMA_FLAG;
			i4RetValue = i2c_master_send(g_pstI2Cclient, g_uI2CDMAPhysicalAddr, a_sizeSendData);
		    if (i4RetValue != a_sizeSendData) {
		        PK_DBG("[CAMERA SENSOR] I2C send failed!!, Addr = 0x%x\n", a_pSendData[0]);
		        return -1;
		    }
		...
		{
			g_pstI2Cclient->ext_flag |= I2C_DMA_FLAG;
			i4RetValue = i2c_master_recv(g_pstI2Cclient, (char *)g_uI2CDMAPhysicalAddr, a_sizeRecvData);
		    if (i4RetValue != a_sizeRecvData) {
		        PK_DBG("[CAMERA SENSOR] I2C read failed!! \n");
		        return -1;
		    }

			memcpy(a_pRecvData, g_pI2CDMAVirtualAddr, a_sizeRecvData);
		}
    }
    else{
    	spin_lock(&kdsensor_drv_lock);
...
MINT32 i = 0;
    ...
    for ( i = (KDIMGSENSOR_MAX_INVOKE_DRIVERS-1) ; i >= KDIMGSENSOR_INVOKE_DRIVER_0 ; i-- ) {
        if ( g_bEnableDriver[i] && g_pInvokeSensorFunc[i] ) {
            // turn on power
            printk("hufeng debug kd_MultiSensorOpen\n");
			...
			if( (DUAL_CAMERA_MAIN_2_SENSOR == g_invokeSocketIdx[i])||(DUAL_CAMERA_SUB_SENSOR==g_invokeSocketIdx[i]) ) {
				spin_lock(&kdsensor_drv_lock);
				gI2CBusNum = SUPPORT_I2C_BUS_NUM2;
				spin_unlock(&kdsensor_drv_lock);
				 printk("kd_MultiSensorOpen: switch I2C BUS2\n");
			}
			else {
				spin_lock(&kdsensor_drv_lock);
...
{
	unsigned int uFeatureLen = 4;
	if (g_pSensorFunc) 
	{
		g_pSensorFunc->SensorFeatureControl(DUAL_CAMERA_MAIN_SENSOR, SENSOR_FEATURE_SET_ISP_DEBUG_FLAG, (unsigned char*)pDebugInfo, &uFeatureLen);
	}
	else
	{
	...
	switch (pFeatureCtrl->FeatureId)
	{
	 ...
	 case SENSOR_FEATURE_SET_SENSOR_SYNC:    // Update new sensor exposure time and gain to keep
		...
		case SENSOR_FEATURE_SILNET_PREVIEW:
			PK_DBG("[CAMERA_HW][pFeaturePara] ioctl SENSOR_FEATURE_SILNET_PREVIEW ,InvokeCamera:0x%x, FeatureId:0x%x\n",pFeatureCtrl->InvokeCamera, pFeatureCtrl->FeatureId);
			break;
        #endif /*QUICK_CAPTURE*/
		//LiuBin@MtkCamera, 2013/04/23, Add for set isp to capture
...
static int CAMERA_HW_i2c_remove(struct i2c_client *client)
{
    ...
    if(g_pI2CDMAVirtualAddr)
    {
		dma_free_coherent(NULL, 0x1b30+8*29, g_pI2CDMAVirtualAddr, g_uI2CDMAPhysicalAddr);
