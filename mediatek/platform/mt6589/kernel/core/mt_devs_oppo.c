...
#ifdef OPPO_R819
//yang.li@oppo.com ,2013/04/06, Add for have unique serial ID
#define CONFIG_MTK_USB_UNIQUE_SERIAL
//Bingshu.Zhou@Prd.SysSrv.USB, 2012/08/08, Add for devices ID
#if defined(OPPO_ULIKE_SERIAL_NUMBER)
	char oppo_serial_number[SERIALNO_LEN] = {"0000012035ABCDEF"};
#elif defined(OPPO_REAL_SERIAL_NUMBER)
	char oppo_serial_number[SERIALNO_LEN] = {"0000012083ABCDEF"};
#else	
	char oppo_serial_number[SERIALNO_LEN] = {"0000013009ABCDEF"};
#endif
#endif /* OPPO_R819 */
#ifdef OPPO_R819
...
#ifdef OPPO_R819   #Nanwei.Deng@Basic.drv, 2013/05/12, Add for 13003 breath_led
static struct platform_device sn3193_pdev = {
...
/*=======================================================================*/
#ifdef OPPO_R819//zhye add for muti gsensor
#if 0
...
#endif /*OPPO_R819*/
#ifndef OPPO_R819
//lizhanhua@Pld.BasicDriver.Sensor,2012/12/18,add mmc3416x
...
struct platform_device sensor_msensor_yas533 = {
	.name	       = "msensor_yas533",
	.id            = -1,
};
#endif
...
#ifdef OPPO_R819//zhye add for 13003
static struct platform_device capswitch_device = {
	.name = "capswitch",
	.id = -1,
};
...
#ifdef OPPO_R819//zhye add for  13009 alsps
#if 0
struct platform_device sensor_alsps_tmd2772 = {
	.name	       = "als_ps_tmd2772",
	.id            = -1,
};
struct platform_device sensor_alsps_ltr558 = {
	.name	       = "als_ps_ltr558",
	.id            = -1,
};
#endif
...
struct platform_device sensor_alsps_stk3171 = {
        .name          = "als_ps_stk3171",
        .id            = -1,
};
struct platform_device sensor_alsps_stk3x1x = {
	.name	       = "als_ps_stk3x1x",
	.id            = -1,
};
...
struct platform_device sensor_alsps_apds9930 = {
...
__init int mt6589_board_init(void)
{
...

	{
		...
		if (key != 0)
    		...
    		#ifndef OPPO_R819
    		//Bingshu.Zhou@Prd.SysSrv.USB, 2012/08/31, Modify for serial_number
    		   memcpy(serial_number, "0123456789ABCDEF", 16);
    		#else /* OPPO_R819 */
			memcpy(serial_number, oppo_serial_number, 16);
    		#endif /* OPPO_R819 */
...
#endif
#ifdef OPPO_R819
/* hanbin.lin@MutimediaDrv.audio, 2013/04/9, Remove for headset */
#if defined(CONFIG_MTK_ACCDET)
...
/* hanbin.lin@MutimediaDrv.audio, 2013/04/9, Remove for headset */
...
#ifdef OPPO_R819//zhye add  for 13009 gsensor
  ...
  #ifdef OPPO_R819//zhangye add for  13009 msensor
	...
	printk("sensor_msensor_mmc3416x  device! retval=%d\n",retval);
	if (retval != 0)
		return retval;
	retval = platform_device_register(&sensor_msensor_yas533);
		printk("sensor_msensor_yas533  device! retval=%d\n",retval);
	if (retval != 0)
		return retval;
  #else
...
#ifdef OPPO_R819
	retval = platform_device_register(&capswitch_device);
		printk("capswitch_device!\n");
	if (retval != 0)
		return retval;
	...
	if (retval != 0)
	 ...
	 retval = platform_device_register(&sensor_alsps_stk31xx);
        ...
        if (retval != 0)
            return retval;
        retval = platform_device_register(&sensor_alsps_stk3171);
        printk("sensor_alsps_stk3171 device !\n");
        if (retval != 0)
            return retval;
	retval = platform_device_register(&sensor_alsps_stk3x1x);
        printk("sensor_alsps_stk3x1x device!\n");
        if (retval != 0)
            return retval;
	...
	//lizhanhua add
	 ...
	 retval = platform_device_register(&sensor_alsps_apds9930);
        printk("sensor_alsps_apds9930 device!");
        if (retval != 0)
            return retval;		
    #else
...
#ifdef OPPO_R819   #Nanwei.Deng@Basic.drv, 2013/05/12, Add for 13003 breath_led
	printk("breath_led register\n");
	retval = platform_device_register(&sn3193_pdev);
	if (retval != 0) {
		return retval;
	}
#endif
