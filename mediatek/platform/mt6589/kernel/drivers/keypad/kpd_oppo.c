...
#ifdef VENDOR_EDIT	//huqiao modify for shield key when TP is working
...
#ifdef VENDOR_EDIT
//redong.shi@BasicDrv.KPD, add 2012/8/6 for prj 12033 volume key 
#ifdef JDI_HD_ACX457AKP //huqiao add for 13025 keypad
#define PMIC_ENT      192
#define GPIO_VOLUMEKEY_UP		(GPIOEXT16)
#define GPIO_VOLUMEKEY_DOWN		(GPIOEXT31)
#define GPIO_HALL	(116)
#define EINT_HALL_NUM   (11)
#define EINT_VOLUMEKEY_UP_NUM		(10 + PMIC_ENT)
#define EINT_VOLUMEKEY_DOWN_NUM		(3 + PMIC_ENT)
#define KEY_UNPRESS			(0)
#define KEY_PRESS			(1)


static void kpd_volumekey_up_eint(void);
...
static void kpd_volumekey_down_eint(void);
...
static void kpd_hall_eint(void);
static void kpd_hall_handler(unsigned long data);
static DECLARE_TASKLET(kpd_hall_tasklet, kpd_hall_handler, 0);

static void kpd_volumekey_up_handler(unsigned long data)
{
	int volumekey_up_press = KEY_UNPRESS;

	volumekey_up_press = !mt_get_gpio_in(GPIO_VOLUMEKEY_UP);
    					   
	input_report_key(kpd_input_dev, KEY_VOLUMEUP, volumekey_up_press);  
	input_sync(kpd_input_dev);
	printk(KERN_ALERT"%s is called.", __func__);
}
static void kpd_volumekey_up_eint(void)
{
	int tmp ;

	tmp = mt_get_gpio_in(GPIO_VOLUMEKEY_UP);
	mt65xx_eint_set_polarity(EINT_VOLUMEKEY_UP_NUM, !tmp);
	mt65xx_eint_unmask(EINT_VOLUMEKEY_UP_NUM);
	tasklet_schedule(&kpd_volumekey_up_tasklet);
	printk(KERN_ALERT"%s is called.", __func__);
}
static void kpd_volumekey_down_handler(unsigned long data)
{
	int volumekey_down_press = KEY_UNPRESS;

	volumekey_down_press = !mt_get_gpio_in(GPIO_VOLUMEKEY_DOWN);
	input_report_key(kpd_input_dev, KEY_VOLUMEDOWN, volumekey_down_press);
	input_sync(kpd_input_dev);
	printk(KERN_ALERT"%s is called.", __func__);
}
static void kpd_volumekey_down_eint(void)
{
	...
	tmp = mt_get_gpio_in(GPIO_VOLUMEKEY_DOWN);
	mt65xx_eint_set_polarity(EINT_VOLUMEKEY_DOWN_NUM, !tmp);
	mt65xx_eint_unmask(EINT_VOLUMEKEY_DOWN_NUM);
...
static void kpd_hall_handler(unsigned long data)
{
	int hall_press = KEY_UNPRESS;
	input_report_key(kpd_input_dev, KEY_POWER, 1);
	input_sync(kpd_input_dev);
	input_report_key(kpd_input_dev, KEY_POWER, 0);
	input_sync(kpd_input_dev);
	printk(KERN_ALERT"%s shirendong is called.", __func__);
}
static void kpd_hall_eint(void)
{
	int tmp ;

	tmp = mt_get_gpio_in(GPIO_HALL);
	mt65xx_eint_set_polarity(EINT_HALL_NUM, !tmp);
	mt65xx_eint_unmask(EINT_HALL_NUM);
...
#ifdef VENDOR_EDIT  //huqiao@BasicDrv.KPD, add 2013/4/15 for prj 13025  key 
#define TOUCH_KEY
#endif /*VENDOR_EDIT*/
#ifdef   TOUCH_KEY
#define GPIO_BACK	(GPIO74)
#define GPIO_HOME_DVT	(GPIO75)
#define GPIO_MENU	    (GPIO73)
#define EINT_BACK_NUM	(74)
#define EINT_HOME_NUM_DVT	(75)
...
static void kpd_back_handler(unsigned long data)
{
 	int volumekey_up_press = KEY_UNPRESS;
        ...
        volumekey_up_press = !mt_get_gpio_in(GPIO_BACK); 
               
        input_report_key(kpd_input_dev, KEY_BACK, volumekey_up_press); 	
		input_sync(kpd_input_dev);
		printk(KERN_ALERT"%s is called.", __func__);
...
static void kpd_home_handler(unsigned long data)
{
 	int volumekey_down_press = KEY_UNPRESS;
        ...
        volumekey_down_press = !mt_get_gpio_in(GPIO_HOME_DVT);
                                    
        input_report_key(kpd_input_dev, KEY_HOME, volumekey_down_press); 
		input_sync(kpd_input_dev);	
		printk(KERN_ALERT"%s is called.", __func__);
...
static void kpd_home_eint(void)
{
	...
	tmp = mt_get_gpio_in(GPIO_HOME_DVT);            
	mt65xx_eint_set_polarity(EINT_HOME_NUM_DVT, !tmp);
	mt65xx_eint_unmask(EINT_HOME_NUM_DVT);
...
static void kpd_menu_handler(unsigned long data)
{
 	int volumekey_down_press = KEY_UNPRESS;
        ...
        volumekey_down_press = !mt_get_gpio_in(GPIO_MENU); 
               
        input_report_key(kpd_input_dev, KEY_MENU, volumekey_down_press); 
		input_sync(kpd_input_dev);	
		printk(KERN_ALERT"%s is called.", __func__);
}
static void kpd_menu_eint(void)
...
#endif //#ifdef   TOUCH_KEY
#endif  // #ifdef JDI_HD_ACX457AKP

#ifdef HT_WVGA_13009   // key for 13009
#define PMIC_ENT                                    192
#define GPIO_VOLUMEKEY_UP                   (GPIOEXT16)
#define GPIO_VOLUMEKEY_DOWN             (GPIOEXT31)
...
#endif//#ifdef HT_WVGA_13009
#ifdef QHD_HX8394_DSI   // key for 13015
#define PMIC_ENT                                    192
#define GPIO_VOLUMEKEY_UP                   (GPIOEXT16)
#define GPIO_VOLUMEKEY_DOWN             (GPIOEXT31)
#define EINT_VOLUMEKEY_UP_NUM           (10+ PMIC_ENT)
...
#endif//#ifdef QHD_HX8394_DSI

#ifdef QHD_HX8394_13003   // key for 13003
#define PMIC_ENT                                    192
#define GPIO_VOLUMEKEY_UP                   (GPIOEXT16)
#define GPIO_VOLUMEKEY_DOWN             (GPIOEXT31)
#define EINT_VOLUMEKEY_UP_NUM           (10+ PMIC_ENT)
...
#endif//#ifdef QHD_HX8394_13003

#ifdef HT_WVGA_13039   // key for 13039
#define PMIC_ENT                                    192
#define GPIO_VOLUMEKEY_UP                   (GPIOEXT16)
#define GPIO_VOLUMEKEY_DOWN             (GPIOEXT31)
#define EINT_VOLUMEKEY_UP_NUM           (10+ PMIC_ENT)
...
#ifndef VENDOR_EDIT//Shaoyu.Huang@BasicDrv.KPD, delete 2012/5/28 for prj 12015 volume key	
...
#ifndef VENDOR_EDIT
//rendong.shi@BasicDrv.kpd  add for long pressing powerkey to  force power-off of PMU
...
#endif	
#else
#if defined(JDI_HD_ACX457AKP)    // 12083
        printk("shirendong long press reboot enable\n");
   		upmu_set_rg_pwrkey_rst_en(0x01);//pmic_config_interface(TOP_RST_MISC, 0x01, PMIC_RG_PWRKEY_RST_EN_MASK, PMIC_RG_PWRKEY_RST_EN_SHIFT);
		upmu_set_rg_homekey_puen(0x01);
		upmu_set_rg_homekey_rst_en(0x00);
		upmu_set_rg_pwrkey_rst_td(0x00);/* timeout period. 0: 5sec; 1: 7sec; 2: 9sec; 3: 11sec */
#elif defined(QHD_HX8394_DSI)   // 13015
        printk("shirendong long press reboot enable\n");
   		upmu_set_rg_pwrkey_rst_en(0x01);//pmic_config_interface(TOP_RST_MISC, 0x01, PMIC_RG_PWRKEY_RST_EN_MASK, PMIC_RG_PWRKEY_RST_EN_SHIFT);
		upmu_set_rg_homekey_puen(0x01);
		upmu_set_rg_homekey_rst_en(0x00);
...
#ifdef VENDOR_EDIT
//Rendong.shi@BasicDrv.KPD, add 2013/4/12 for 12083 volume key wake up
...
/****************************************************************************************/
...
#endif
#ifdef VENDOR_EDIT//Rendong.shi@BasicDrv.KPD, add 2012/8/6 for 12033 volume key
#ifdef JDI_HD_ACX457AKP  // KEY for 13009
	mt_set_gpio_mode(GPIO_VOLUMEKEY_UP, GPIO_MODE_02);
	...
	mt_set_gpio_mode(GPIO_VOLUMEKEY_DOWN, GPIO_MODE_02);
...
#ifndef VENDOR_EDIT
//Fuchun.Liao@wxkfDrv,2013/05/29,delete hall key	
	mt_set_gpio_mode(GPIO_HALL, GPIO_MODE_01);
	mt_set_gpio_dir(GPIO_HALL, GPIO_DIR_IN);
	mt_set_gpio_pull_enable(GPIO_HALL, GPIO_PULL_ENABLE);
	mt_set_gpio_pull_select(GPIO_HALL, GPIO_PULL_UP);
#endif /*VENDOR_EDIT*/
	
	mt65xx_eint_set_sens(EINT_VOLUMEKEY_UP_NUM, CUST_EINT_EDGE_SENSITIVE);
	mt65xx_eint_set_polarity(EINT_VOLUMEKEY_UP_NUM, CUST_EINT_POLARITY_LOW);
	mt65xx_eint_set_hw_debounce(EINT_VOLUMEKEY_UP_NUM,(0XA0 >> 5));
	mt65xx_eint_registration(EINT_VOLUMEKEY_UP_NUM, 
				true, 
	...
	mt65xx_eint_set_sens(EINT_VOLUMEKEY_DOWN_NUM, CUST_EINT_EDGE_SENSITIVE);
	mt65xx_eint_set_polarity(EINT_VOLUMEKEY_DOWN_NUM, CUST_EINT_POLARITY_LOW);
	mt65xx_eint_set_hw_debounce(EINT_VOLUMEKEY_DOWN_NUM, (0XA0 >> 5));
	mt65xx_eint_registration(EINT_VOLUMEKEY_DOWN_NUM, 
				true, 
				CUST_EINT_POLARITY_LOW,
				kpd_volumekey_down_eint, 
				false);
#ifdef VENDOR_EDIT  //huqiao modify for disable hall key
	/*mt65xx_eint_set_sens(EINT_HALL_NUM, CUST_EINT_EDGE_SENSITIVE);
	mt65xx_eint_set_polarity(EINT_HALL_NUM, CUST_EINT_POLARITY_LOW);
	mt65xx_eint_set_hw_debounce(EINT_HALL_NUM, (0XA0 >> 5));
	mt65xx_eint_registration(EINT_HALL_NUM, 
				true, 
				CUST_EINT_POLARITY_LOW,
				kpd_hall_eint, 
				false);	*/		
	...
	mt_set_gpio_mode(GPIO_HOME_DVT, GPIO_MODE_02);
	mt_set_gpio_dir(GPIO_HOME_DVT, GPIO_DIR_IN);
	mt_set_gpio_pull_enable(GPIO_HOME_DVT, GPIO_PULL_ENABLE);
	mt_set_gpio_pull_select(GPIO_HOME_DVT, GPIO_PULL_UP);
	...
	mt65xx_eint_registration(EINT_BACK_NUM, 
					...
					CUST_EINT_POLARITY_LOW,
	                         	kpd_back_eint, 
					false);
	mt65xx_eint_set_sens(EINT_HOME_NUM_DVT, CUST_EINT_EDGE_SENSITIVE);
	mt65xx_eint_set_polarity(EINT_HOME_NUM_DVT, CUST_EINT_POLARITY_LOW);
	mt65xx_eint_set_hw_debounce(EINT_HOME_NUM_DVT, (0XA0 >> 5));
	mt65xx_eint_registration(EINT_HOME_NUM_DVT, 
					true, 
					CUST_EINT_POLARITY_LOW,
	                         	kpd_home_eint, 
					false);
	...
	__set_bit(KEY_MENU, kpd_input_dev->keybit);
#endif //#ifdef TOUCH_KEY
#endif//#ifdef JDI_HD_ACX457AKP 
...
#ifdef VENDOR_EDIT
//Rendong.shi@BasicDrv.KPD, add 2013/4/12 for 12083 volume key wake up
#ifdef JDI_HD_ACX457AKP  
//#define MTK_KP_WAKESOURCE//this is for auto set wake up source
#elif defined(QHD_HX8394_DSI)   // KEY for 13015
//#define MTK_KP_WAKESOURCE//this is for auto set wake up source
#else
